...
def main():
 ...
 '## Reproduce','', '```powershell', '$env:HF_HOME = Join-Path (Get-Location) ".cache/decision-engine/huggingface"', '.\\.cache\\decision-engine\\venv\\Scripts\\python.exe -m ora_decision_engine --workflow examples/decision_engine/router.workflow.json --interactive --device cuda --local-files-only --answer-bias contextual', '```','',
