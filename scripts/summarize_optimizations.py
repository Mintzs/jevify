...
'- src/ora_decision_engine/engine.py: configuration, cache fork/head-row reuse, graph dispatch, model-scoped serialization and metadata.','- src/ora_decision_engine/cuda_graphs.py: exact-shape input buffers, capture/replay, bounded storage, memory admission and fallback.','- src/ora_decision_engine/kernels.py: custom Triton RMSNorm, SiLU-plus-multiplication (SwiGLU), and query/key RoPE. Intermediate low-precision rounding is retained where possible.','- src/ora_decision_engine/fusion.py: model-local adapters; no global Transformers patch. Fusion is validated for dense Qwen2 with Transformers 5.5.4.','- tests/test_optimizations.py: cache isolation/invalidation, changed-input graph replay, masks, fallback, eviction, CPU fallback, kernel numerical checks and all-fusion graph integration.','- docs/optimizations.md: API/CLI usage, dependency requirements, memory behavior and limitations.','',
...
manifest={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest()for p in (ROOT/'src/ora_decision_engine').glob('*.py')}
