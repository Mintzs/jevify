...
def main():
    ...
    (OUT/"production-source-manifest.json").write_text(json.dumps({str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in (ROOT/"src/ora_decision_engine").glob("*.py")},indent=2),encoding="utf-8")
