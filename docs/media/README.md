...
[Watch or download the MP4](jevify-comparison.mp4). Silent, 11 seconds, 1920 × 1080, 60 fps.
...
|---|---:|---|
| Jevify | 336.97 ms | Correct route, valid schema |
...
The replay runs at 1× speed after a two-second introduction. Base-model text appears at its recorded token-arrival times. Jevify and the HF implementation display assembled JSON when their requests finish, using the comparison adapter's common answer representation. Probabilities and base output text are unchanged. Frame timing introduces at most 16.67 ms of rounding.

Jevify used CUDA graphs off and RMSNorm, SwiGLU, and RoPE fusions on. Capture code: [capture_json_video_trace_v2.py](../../scripts/capture_json_video_trace_v2.py).
