...
These are local measurements of Jevify, the pinned Hugging Face implementation, and ordinary Qwen JSON generation. They do not compare against Jev itself or establish general model accuracy.
...
| Jevify, CUDA graphs on | 225/250 (90.0%) | 250/250 (100%) | 950.695 ms |
| Jevify, CUDA graphs off | 225/250 (90.0%) | 250/250 (100%) | 167.107 ms |
...
- Checkpoint: `Qwen/Qwen2.5-1.5B-Instruct`, revision `989aa7980e4cf806f80c7fef2b1adb7bc71aa306`, FP16 backbone. Jevify scores the selected output-head rows in FP32.
...
- Jevify settings: delimited prompts, letter option IDs, efficient cache enabled, RMSNorm/SwiGLU/RoPE fusions enabled, SDPA attention, branch batch size 8. CUDA graphs disabled for the headline result. The README's basic installation does not enable optional fusions automatically.
...
The ordinary Qwen baseline generates that JSON without constrained decoding or repair. Its 9.2% therefore measures structured-task success, not standalone classification accuracy. Jevify and the HF implementation assemble their JSON from model scores. The observed 100% schema validity is separate from answer correctness or probability calibration.
