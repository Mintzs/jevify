...
```powershell
.\.cache\decision-engine\venv\Scripts\python.exe -m ora_decision_engine --workflow examples/decision_engine/router.workflow.json --interactive --device cuda --local-files-only
```
...
```powershell
.\.cache\decision-engine\venv\Scripts\python.exe -m ora_decision_engine --workflow examples/decision_engine/router.workflow.json --prompt "Which model should answer this?" --context "Write and debug a Python CSV parser." --device cuda --local-files-only
```
