...
```bash
ora-decision --device cuda --cuda-graphs --fused-kernels rmsnorm swiglu rope --input workflow.json
```
...
The local benchmark environment is under ~/.cache/ora-decision-engine/profiling-20260918. scripts/run_optimization.sh sets its isolated compiler and launches benchmarks. This launcher is a convenience for this workstation, not a portable installer.
