...
from ora_decision_engine import Workflow
from ora_decision_engine.__main__ import main
...
class WorkflowTests(unittest.TestCase):
    ...
    def test_cli_request_and_legacy_are_supported(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            for args, context in ((["--workflow", str(workflow), "--request", str(request)], "New input"),
                ...
                engine = RecordingEngine()
                with patch("sys.argv", ["ora-decision", *args]), patch("sys.stdout", new_callable=io.StringIO), patch("ora_decision_engine.__main__.build_engine", return_value=engine) as loader:
                    main()
    ...
    def test_cli_bad_request_does_not_load_model_or_inherit_old_context(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch("sys.argv", ["ora-decision", "--workflow", str(path), "--prompt", "Route?"]), patch("sys.stderr", new_callable=io.StringIO), patch("ora_decision_engine.__main__.build_engine") as loader:
    ...
    def test_interactive_loads_once_and_resets_request_prompts(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch("sys.argv", ["ora-decision", "--workflow", str(path), "--interactive"]), patch("builtins.input", side_effect=entries), patch("sys.stdout", new_callable=io.StringIO), patch("ora_decision_engine.__main__.build_engine", return_value=engine) as loader:
                main()
            loader.assert_called_once()
